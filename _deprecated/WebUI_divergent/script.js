...
window.dispatchJuceEvent = (name, data) => {
    console.log(`[JS Bridge] Received Event: ${name}`, data);
    if (eventListenersInternal[name]) {
        eventListenersInternal[name].forEach(fn => fn(data));
    }
};
window.onJuceEvent = window.dispatchJuceEvent;
...
// [Build 22/23] Global bridge convenience for index.html handlers (only create if not already defined)
if (!window.juce || !window.juce.isWasmMock) {
    window.juce = {
        ...
        pianoNoteOn: (note, vel) => callNative("pianoNoteOn", note, vel),
