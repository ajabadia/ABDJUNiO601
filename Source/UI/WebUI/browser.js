...
const PresetBrowser = {
    ...
    async importLibrary() {
        // Route through Smart Import modal via handleImportJson
        try {
            await juce.menuAction('handleImportJson');
            // The Smart Import modal will handle the file selection, analysis,
            // and import confirmation. After import, onImportResult will trigger
            // a refresh via the existing listener in setupListeners().
        } catch (e) {
    ...
    sortBy(column) {
        ...
        document.querySelectorAll('.preset-col-headers .col-sort').forEach(el => {
            const col = el.getAttribute('data-sort');
            el.classList.toggle('active', col === column);
            el.classList.toggle('asc', col === column && this.sortAscending);
            el.classList.toggle('desc', col === column && !this.sortAscending);
            const arrow = el.querySelector('.sort-arrow');
            if (arrow) {
                arrow.textContent = col === column ? (this.sortAscending ? '▲' : '▼') : '';
            }
        });
        this.renderPresets();
    },
    ...
    handleListKeydown(e, type) {
        // Escape closes the browser for any list type
        if (e.key === 'Escape') {
            hideBrowser();
            return;
        }
        ...
        if (type === 'preset') {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                const active = document.querySelector('#preset-list .active');
        ...
        if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
            ...
            const listId = type === 'category' ? '#cat-list' : type === 'library' ? '#lib-list' : '#preset-list';
            ...
            // Wrap around (cat/lib) or clamp (preset)
            if (type === 'preset') {
    ...
    renderPresets() {
        ...
        // Save selected preset name for restoration after re-render
        const prevSelectedName = this.selectedPresetIdx >= 0 && lib.patches[this.selectedPresetIdx]
            ? lib.patches[this.selectedPresetIdx].name : null;
        ...
        lib.patches.forEach((p, idx) => {
            ...
            if (matchesSearch && matchesCat) {
                filtered.push({ patch: p, origIdx: idx });
            }
        });
        ...
        filtered.sort((a, b) => {
            ...
            switch (this.sortColumn) {
                case 'name':
                    cmp = a.patch.name.localeCompare(b.patch.name);
                    break;
                case 'category':
                    cmp = (a.patch.category || '').localeCompare(b.patch.category || '');
                    if (cmp === 0) cmp = a.patch.name.localeCompare(b.patch.name);
                    break;
                case 'library':
                    cmp = (lib.name || '').localeCompare(lib.name || '');
                    if (cmp === 0) cmp = a.patch.name.localeCompare(b.patch.name);
                    break;
                case 'favorite':
                    cmp = (b.patch.favorite ? 1 : 0) - (a.patch.favorite ? 1 : 0);
                    if (cmp === 0) cmp = a.patch.name.localeCompare(b.patch.name);
                    break;
            }
        ...
        filtered.forEach((item, displayIdx) => {
            ...
            // Restore selection by name match (robust across filter/sort changes)
            if (prevSelectedName && p.name === prevSelectedName) {
                restoredIdx = displayIdx;
                this.selectedPresetIdx = origIdx;
            }

            const li = document.createElement('li');
            li.className = 'preset-item';
            li.dataset.origIdx = origIdx;
            if (this.selectedPresetIdx === origIdx || restoredIdx === displayIdx) {
            ...
            catSpan.className = 'preset-cat-label';
            catSpan.textContent = p.category || '';
            leftSide.appendChild(catSpan);
            ...
            favBtn.textContent = '★';
            favBtn.onclick = (e) => this.toggleFavorite(e, this.selectedLibIdx, origIdx);
        ...
        // Update count
        const countLabel = document.getElementById('preset-count-label');
        if (countLabel) countLabel.innerText = filtered.length + ' Presets';
    },
