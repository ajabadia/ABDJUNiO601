...
public:
    ...
    {
        // === BOTONES RANGE (TextButton) ===
        // Look like buttons 16' 8' 4'
        auto cfgBtn = [&](juce::TextButton& b, const char* txt) {
            b.setButtonText(txt);
            b.setClickingTogglesState(true);
            b.setRadioGroupId(101); // Mutual exclusion
        ...
        // Setup Waves (ToggleButtons but styled)
        ...
        apvts.addParameterListener("pulseOn", this); // FIX: Listen for LEDs
        apvts.addParameterListener("sawOn", this);   // FIX: Listen for LEDs
        updateRangeUI(); 
    }
    ...
    void handleAsyncUpdate() override { 
        updateRangeUI(); 
        repaint(); // FIX: Force repaint for custom LEDs
    }
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        drawHdr("MODE", 2, 0, colW/2); // Split MODE col
        drawHdr("WAVE", 2, colW/2, colW/2); // New header for waves
        ...
        // Helper to draw LED
        auto drawLED = [&](juce::Component& comp, bool active = false) { 
             if (comp.isVisible()) {
                 ...
                 if (on) {
                     g.setColour(juce::Colour(0xffff3030)); // Red ON
                     ...
                     g.setColour(juce::Colour(0xff502020)); // Dim Red OFF
                     g.fillEllipse(x+1, y+1, ledSize-2, ledSize-2);
                 }
             }
        };
    ...
    {
        auto r = getLocalBounds().reduced(0, 28); // Skip header (28px now)
        float colW = (float)getWidth() / 4.0f; 
        ...
        auto placeSlider = [&](juce::Slider& s, float xOffset) {
            int cx = (int)(xOffset + (subColW - sliderW)/2);
            s.setBounds(cx, yControls, sliderW, sliderH);
        };
        placeSlider(lfoSlider, col2Start);
        placeSlider(pwmSlider, col2Start + subColW);

        // --- COL 3: PWM MODE & WAVES ---
        // Mode Switch (LFO/MAN) on left subcol, Waves on right subcol?
        // Or Mode Switch under PWM slider in Col 2? 
        // User pointed to "switch near slider".
        // Let's put PWM Switch in Col 3 Left (Mode), Waves in Col 3 Right (Wave)
        ...
        // Waves
        int waveBtnSize = 30;
        int wx = (int)(col3Start + subColW + (subColW - waveBtnSize)/2);
        // Stacked or side-by-side? Side by side fits better in wider col.
        // Actually Col 3 is "MODE(PWM)" and "WAVE".
        // Let's put waves side-by-side.
        int wGap = 10;
        int wTotal = (waveBtnSize * 2) + wGap;
        int wxStart = (int)(col3Start + subColW + (subColW - wTotal)/2);
